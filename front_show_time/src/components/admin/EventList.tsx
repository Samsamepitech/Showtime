      ...
      <div className="AdminSideBar1">
        ...
        <table className="alluTable">
          ...
          <tr className="widgetLgTr">
              ...
              <button type="button" className="btn btn-info" onClick={() =>  axios.put(`http://localhost:3030/concerts/update/${concert._id}`)}/>
            </Link> </p>})} </td>
              <td className="deleteC">{concerts.map((concert : any)=>{
              return <p key={concert.id}> <Link to={{pathname: "/admin" }}> <button  type="button" className="btn btn-danger" onClick={() =>  axios.delete(`http://localhost:3030/concerts/${concert._id}`)}/> </Link> </p>})} </td>
          </tr>
        </table>
      </div>
      </div>
