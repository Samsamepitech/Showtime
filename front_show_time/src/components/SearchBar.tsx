...
export default function Concert() {
  ...
  getConcerts()
    ...
    return (
      <>
      <Col xs={4}>
      <Form.Control placeholder="Filter by artist or date or place" onChange={event => setQuery(event.target.value)}  />
      </Col>
       {concerts.filter(concert => {
    if (query === '') {
      return;
    } else if (concert.artist.toLowerCase().includes(query.toLowerCase()) || concert.place.toLowerCase().includes(query.toLowerCase())) {
      return concerts;
    }
